...
class UploadService {
    async enforceUserActiveUploadQuota(userId, increment = 1) {
        const activeCount = await Video_js_1.Video.countDocuments({
            userId,
            status: { $in: ACTIVE_UPLOAD_STATUSES },
        });
        if (activeCount + increment > MAX_ACTIVE_UPLOADS_PER_USER) {
            throw new Error(`Active upload limit reached (${MAX_ACTIVE_UPLOADS_PER_USER}). Complete or remove existing uploads before adding more.`);
        }
    }
    async initializeUploadBatch(userId, userType, items) {
     ...
     */
    setStatusWithHistory(video, to, reason) {
     ...
     * Initialize Upload
     */
    async initializeUpload(userId, userType, data, options) {
        ...
        const presignedData = await r2Service_js_1.r2Service.getUploadPresignedUrl(userId, videoId, filename, normalizedMimeType, userType);
        const presignedUrlExpiresAt = new Date(Date.now() + presignedData.expiresIn * 1000);
        if (!video) {
            // Create new
            video = new Video_js_1.Video({
                videoId,
                userId,
                userType,
                title,
    ...
    async completeUpload(userId, videoId) {
        ...
        if (video.status !== 'pending')
            throw new Error(`Invalid status: ${video.status}. Expected: pending`);
        if (video.presignedUrlExpiresAt && new Date() > video.presignedUrlExpiresAt) {
            throw new Error('Upload URL expired');
        }
        // Verify R2
        const fileExists = await r2Service_js_1.r2Service.fileExists(r2_js_1.R2_BUCKETS.RAW, video.originalFile.r2Key);
        if (!fileExists)
            throw new Error('File not found in storage');
        // Update Metadata
        const metadata = await r2Service_js_1.r2Service.getFileMetadata(r2_js_1.R2_BUCKETS.RAW, video.originalFile.r2Key);
        ...
        const qualities = video.userType === 'creator'
            ...
            : ['720p', '480p', '360p', '240p'];
        const jobId = await queueService_js_1.queueService.addTranscodeJob({
    ...
    async getStatus(userId, videoId) {
        ...
        if (video.transcoding?.jobId && video.status === 'processing') {
            try {
                jobStatus = await queueService_js_1.queueService.getJobStatus(video.videoId, video.userType);
            }
            catch { }
        }
        return {
     ...
     */
    async listCreatorVideos(userId, params) {
        ...
        const [videos, total] = await Promise.all([
            Video_js_1.Video.find(query).sort({ createdAt: -1 }).skip(skip).limit(params.limit).lean(),
     ...
     * Retry Transcoding
     */
    async retryTranscoding(userId, videoId) {
        const video = await Video_js_1.Video.findOne({ videoId });
        if (!video)
            throw new Error('Video not found');
        if (video.userId !== userId)
        ...
        // Atomic update
        const updated = await Video_js_1.Video.findOneAndUpdate({ videoId, userId, status: 'failed' }, {
        ...
        if (!updated) {
            ...
            if (video.status !== 'failed')
                throw new Error(`Cannot retry status: ${video.status}`);
            throw new Error('Retry failed (concurrency)');
        }
        // Verify R2
        const fileExists = await r2Service_js_1.r2Service.fileExists(r2_js_1.R2_BUCKETS.RAW, updated.originalFile.r2Key);
        if (!fileExists) {
            await Video_js_1.Video.updateOne({ videoId }, { $set: { status: 'failed', 'transcoding.error': 'File missing' } });
        ...
        await Video_js_1.Video.updateOne({ videoId }, { $set: { 'transcoding.jobId': jobId } });
        return { videoId, status: 'queued', jobId, message: 'Transcoding restarted' };
    }
    /**
     * Get Raw URL
     */
    async getRawUrl(userId, videoId) {
        const video = await Video_js_1.Video.findOne({ videoId });
        if (!video)
            throw new Error('Video not found');
        if (video.userId !== userId)
            throw new Error('Unauthorized');
        const rawUrl = await r2Service_js_1.r2Service.getDownloadPresignedUrl(r2_js_1.R2_BUCKETS.RAW, video.originalFile.r2Key, 3600);
        return {
            videoId,
            rawUrl,
            expiresIn: 3600,
    ...
    async deleteVideo(userId, videoId) {
        ...
        if (video.status === 'processing')
            throw new Error('Cannot delete processing video');
