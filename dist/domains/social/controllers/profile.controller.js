...
class ProfileController {
    ...
    async uploadAvatarDirect(req, res) {
        try {
            const { userId } = (0, authHelpers_js_1.ensureAuthenticatedUser)(req);
            ...
            const result = await profileService.uploadAssetDirect(userId, 'avatar', mimeType, fileBuffer);
    ...
    async uploadBannerDirect(req, res) {
        try {
            const { userId } = (0, authHelpers_js_1.ensureAuthenticatedUser)(req);
            const mimeType = String(req.headers['content-type'] || '').split(';')[0].trim();
            if (!ALLOWED_IMAGE_MIME_TYPES.has(mimeType)) {
            ...
            const result = await profileService.uploadAssetDirect(userId, 'banner', mimeType, fileBuffer);
            return res.status(200).json({ success: true, data: result });
        }
        catch (error) {
