...
const VideoAnalytics_js_1 = require("../models/VideoAnalytics.js");
const UserAnalytics_js_1 = require("../models/UserAnalytics.js");
const env_js_1 = require("../config/env.js");
// ─── Configuration ──────────────────────────────────────
const REDIS_KEY = 'vm:analytics:events';
const FLUSH_INTERVAL_MS = (0, env_js_1.getNumericEnv)('ANALYTICS_WORKER_FLUSH_MS', 60000); // 60 seconds
const MAX_EVENTS_PER_FLUSH = (0, env_js_1.getNumericEnv)('ANALYTICS_WORKER_MAX_EVENTS', 5000); // Max events per cycle
// ─── Push Events to Redis ───────────────────────────────
/**
 * Push a batch of events to the Redis list.
 * Called by the events API endpoint.
 */
async function pushEventsToRedis(events) {
 ...
 */
async function updateActiveSession(event) {
    if (event.type !== 'heartbeat' && event.type !== 'play' && event.type !== 'watchtime') {
        return;
    }
    try {
        await VideoAnalytics_js_1.ActiveSession.findOneAndUpdate({ sessionId: event.sessionId }, {
            $set: {
...
async function removeActiveSession(sessionId) {
    try {
        await VideoAnalytics_js_1.ActiveSession.deleteOne({ sessionId }).exec();
    }
    catch {
        // Ignore
    }
...
async function aggregateAndWrite(events) {
    ...
    for (const event of events) {
        ...
        switch (type) {
            case 'play':
                vStats.plays++;
                if (data.duration)
                    vStats.durations.push(data.duration);
                break;
            case 'pause':
                vStats.pauses++;
                break;
            ...
            case 'ended':
                if (data.completionRate)
                    vStats.completionRates.push(data.completionRate);
                if (data.watchTime)
                    vStats.watchTime += data.watchTime;
                break;
            case 'watchtime':
                if (data.watchTime)
                    vStats.watchTime += data.watchTime;
        ...
        switch (type) {
            ...
            case 'watchtime':
                if (data.watchTime)
                    uStats.watchTimeAdded += data.watchTime;
                if (data.currentTime) {
                    uStats.lastPosition = data.currentTime;
                    uStats.maxPosition = Math.max(uStats.maxPosition, data.currentTime);
                }
                if (data.duration)
                    uStats.duration = data.duration;
                if (data.completionRate)
                    uStats.completionRate = Math.max(uStats.completionRate, data.completionRate);
                if (data.quality)
                    uStats.quality = data.quality;
                if (data.videoType)
                    uStats.videoType = data.videoType;
                break;
            case 'ended':
                if (data.watchTime)
                    uStats.watchTimeAdded += data.watchTime;
    ...
    for (const [videoId, stats] of videoStats) {
        const avgCompletionRate = stats.completionRates.length > 0
            ...
            : 0;
        writePromises.push(VideoAnalytics_js_1.VideoAnalytics.findOneAndUpdate({ videoId, date: today }, {
            $inc: {
    ...
    for (const [key, stats] of userVideoStats) {
        ...
        writePromises.push(UserAnalytics_js_1.UserWatchHistory.findOneAndUpdate({ userId, videoId }, {
            $inc: {
    ...
    for (const [userId, sessionIds] of userSessions) {
        ...
        writePromises.push((async () => {
            try {
                ...
                const existing = await UserAnalytics_js_1.UserAnalyticsSummary.findOne({ userId }).lean();
                if (existing) {
                    ...
                    if (existing.todayDate !== today) {
                        ...
                        if (existing.lastActiveDate === yesterdayStr) {
                            ...
                            updateOps.$set.longestStreak = Math.max(existing.longestStreak || 0, (existing.currentStreak || 0) + 1);
                        }
                ...
                await UserAnalytics_js_1.UserAnalyticsSummary.findOneAndUpdate({ userId }, updateOps, { upsert: true }).exec();
            }
            catch (err) {
...
//# sourceMappingURL=analyticsWorker.js.map