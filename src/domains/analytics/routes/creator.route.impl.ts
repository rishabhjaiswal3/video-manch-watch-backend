...
router.get('/video/:videoId', async (req: Request, res: Response) => {
  try {
    ...
    const video = await Video.findOne({ videoId, userId }).lean<IVideo>();
...
function buildQualityDistribution(rows: Array<{ _id: string; count: number }>) {
...
function buildContentTypeDistribution(rows: Array<{ _id: string; count: number }>) {
  const distribution: Record<string, number> = {};
...
function buildResolutionDistribution(rows: Array<{ _id: string; count: number }>) {
  const distribution: Record<string, number> = {};
...
function sanitizeOutputs(outputs?: IVideo['outputs']) {
  if (!outputs) return [];
...
function fillTrendWindow(
  rows: Array<{ _id: string; uploaded: number; completed: number; failed: number; totalSize: number }>,
  start: Date,
  days: number
) {
  const map: Record<string, { uploaded: number; completed: number; failed: number; totalSize: number }> = {};
  rows.forEach((row) => {
    map[row._id] = {
